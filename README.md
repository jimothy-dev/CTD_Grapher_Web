...
- **Cast** — one station at a time: every variable against depth on the same picture, each on its own x scale in its own colour, the classic station plot; or one such plot per active station.
